-- =====================================================================
-- Algunas patentes del remito no son de la empresa (autos particulares
-- de empleados que cargan en la misma estación) — hace falta poder
-- marcarlas para que no se les impute costo a ninguna unidad, y que
-- dejen de aparecer como "sin match" en cada importación.
-- =====================================================================

alter table mapeo_patente_externa alter column id_unidad drop not null;
alter table mapeo_patente_externa add column if not exists ignorar boolean not null default false;

alter table mapeo_patente_externa drop constraint if exists mapeo_patente_externa_id_unidad_o_ignorar;
alter table mapeo_patente_externa add constraint mapeo_patente_externa_id_unidad_o_ignorar
  check ((ignorar and id_unidad is null) or (not ignorar and id_unidad is not null));

create or replace function guardar_mapeo_patente_externa(p_patente_texto text, p_id_unidad uuid, p_ignorar boolean default false)
...
begin
  ...
  if p_ignorar then
    insert into mapeo_patente_externa (empresa_id, patente_texto, id_unidad, ignorar, usuario_alta)
    values (v_empresa, v_patente_norm, null, true, (select id from usuarios where auth_user_id = auth.uid()))
    on conflict (empresa_id, patente_texto) do update set id_unidad = null, ignorar = true;
  ...
  insert into mapeo_patente_externa (empresa_id, patente_texto, id_unidad, ignorar, usuario_alta)
  values (v_empresa, v_patente_norm, p_id_unidad, false, (select id from usuarios where auth_user_id = auth.uid()))
  on conflict (empresa_id, patente_texto) do update set id_unidad = excluded.id_unidad, ignorar = false;
...
-- =====================================================================
-- importar_remito_combustible: ahora salta directo las filas cuya
-- patente está marcada como "ignorar" (auto particular), sin contarlas
-- como sin_match ni importarlas a ningún lado.
-- =====================================================================
...
begin
  ...
  for v_fila in select * from jsonb_array_elements(p_filas) loop
    ...
    select id_unidad, ignorar into v_id_unidad, v_ignorar from mapeo_patente_externa
...
grant execute on function guardar_mapeo_patente_externa(text, uuid, boolean) to authenticated;
