-- =====================================================================
-- Agrega el KPI "días operativos totales" a la ficha del activo: días
-- desde el alta de la unidad, menos el total histórico de días que
-- estuvo fuera de servicio (unidad_paradas, sin límite de 12 meses).
-- =====================================================================
...
declare
  ...
  v_dias_parada_total numeric := 0;
  v_dias_operativos_totales numeric := 0;
begin
  select *
  into v_unidad
  from unidades
  ...
  select coalesce(round(sum(extract(epoch from (coalesce(hasta, now()) - desde)) / 86400)::numeric, 1), 0)
  into v_dias_parada_total
  from unidad_paradas
  where id_unidad = p_id_unidad;

  v_dias_operativos_totales := greatest(0, round(extract(epoch from (now() - v_unidad.fecha_alta)) / 86400) - v_dias_parada_total);
  ...
  return jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      ...
      'dias_operativos_totales', v_dias_operativos_totales
    ),
