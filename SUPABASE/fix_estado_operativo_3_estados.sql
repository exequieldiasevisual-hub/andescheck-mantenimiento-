-- ==== estado_operativo pasa a 3 estados en vez de 2 ====
-- Antes: cualquier OT abierta (por chica que sea) hacía decir "En
-- mantenimiento", mezclado con el concepto de "Fuera de servicio" (parada
-- abierta) que es otra cosa. Ahora:
--   - 'Fuera de servicio'   -> tiene una parada abierta (unidad_paradas):
--                              de verdad no puede operar.
--   - 'Con tareas en curso' -> tiene alguna OT abierta pero NO está parada:
--                              sigue operando con trabajo pendiente.
--   - 'Operativa'           -> sin OT abierta y sin parada: 100% libre.
-- Esto deja "estado_operativo" coherente con la métrica de "Unidades
-- operativas" del Dashboard, que ya mide disponibilidad por parada, no por
-- cantidad de OTs.
...
declare
  ...
  v_parada_abierta boolean := false;
begin
  select *
  into v_unidad
  from unidades
  ...
  select
    count(*),
    count(*) filter (where estado_calculado = 'Vencida')
  ...
  select exists(select 1 from unidad_paradas where id_unidad = p_id_unidad and hasta is null)
  into v_parada_abierta;
  ...
  return jsonb_build_object(
    ...
    'estado_operativo', case
      when v_parada_abierta then 'Fuera de servicio'
      when v_ot_abiertas > 0 then 'Con tareas pendientes'
      else 'Operativa'
    end,
