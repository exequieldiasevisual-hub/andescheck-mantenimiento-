-- ==== Dashboard: filtros multi-select (centro de costo, tipo, ciudad) ====
-- get_salud_flota y get_dashboard ahora aceptan 3 arrays opcionales que
-- filtran TODO el dashboard (semaforo, tarjetas y "Para hoy") por atributos
-- de la unidad. Array null o vacío = sin filtro (comportamiento actual).
--
-- IMPORTANTE: hay que borrar las versiones sin argumentos antes de crear
-- estas con parámetros — si no, Postgres las deja como dos funciones
-- distintas superpuestas (mismo bug que ya pasó con crear_ot) y después
-- "Could not choose the best candidate function" en cualquier llamada.

drop function if exists get_salud_flota();
drop function if exists get_dashboard();

create or replace function get_salud_flota(p_centros text[] default null, p_tipos text[] default null, p_ciudades text[] default null)
...
begin
  ...
  return coalesce((
    with r as (
      select id_unidad, count(*) as total, count(*) filter (where estado_calculado = 'Vencida') as vencidas
...
grant execute on function get_salud_flota(text[], text[], text[]) to authenticated;
grant execute on function get_dashboard(text[], text[], text[]) to authenticated;
