...
function ToggleUsarOtChofer({ empresaId, activo, onChange }) {
  async function guardar(e) {
    await supabase.from('configuracion').upsert({ empresa_id: empresaId, seccion: 'parametros', clave: 'usar_ot_chofer', valor: String(e.target.checked) })
    onChange()
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
      <label className="flex items-start gap-2 text-sm cursor-pointer">
        <input type="checkbox" checked={activo} onChange={guardar} className="mt-0.5" />
        <span>
          <span className="font-medium text-gray-900 dark:text-gray-100">OT para choferes</span>
          <span className="block text-xs text-gray-400 mt-0.5">
            El chofer ve "Órdenes de Trabajo" en el menú y puede trabajar las tareas que se le asignen, igual que un técnico (avanzar/completar tareas, checklist de cierre, firma, stock). No puede crear ni cerrar la OT.
          </span>
        </span>
      </label>
    </div>
...
function DestinatariosChecklist({ empresaId, filas, filasUnidades, tiposUnidad, etiquetasTipoUnidad, unidades, onChange }) {
  ...
  async function agregar() {
    ...
    setError('')
    if (!email) return
    if (filas.some(f => f.clave.toLowerCase() === email.toLowerCase())) { setError('Ese mail ya está en la lista'); return }
    const { error: err } = await supabase.from('configuracion').insert({ empresa_id: empresaId, seccion: 'checklist_destinatarios_mail', clave: email, valor: '' })
      ...
      <div className="p-6 max-w-3xl space-y-6">
        {tab === 'General' && (
          <>
            ...
            <ToggleUsarOtChofer empresaId={usuario.empresa_id} activo={parametros.usar_ot_chofer === 'true'} onChange={cargar} />
